...
export function useTokenBalance(symbol, address = '') {
  ...
  const updateBalance = useCallback(() => {
    ...
    Promise.all(balances).then(([balance, spendableBalance]) => {
      if (!cancelled) {
        console.log(balance.toString(), spendableBalance.toString())
  ...
  useEffect(() => {
    ...
    updateBalance()
    console.log(account, address, tokenContract)
    if ((!account && !address) || !tokenContract) {
      return
    }
...
export function useBondingCurvePrice(amount, forwards = true) {
  ...
  console.log('connector', connectorWeight)
  const RETRY_EVERY = 5000

  useEffect(() => {
    let cancelled = false
    let retryTimer
    console.log('bondedContract', bondedContract, collateralTreasuryBalance, bancorContract)
    ...
    const getSalePrice = async () => {
      ...
      try {
        ...
        console.log('getPrice', amountToCheck)
        ...
        if (!cancelled) {
          setLoading(false)
          setPrice(amount.isZero() ? bigNum(0) : salePrice)
          setPricePerUnit(amount.isZero() ? formatUnits(salePrice, { commas: false }) : salePrice/amount)
        }
      } catch (err) {
        if (!cancelled) {
  ...
  }, [amount, bondedContract, collateralTreasuryBalance, bancorContract, connectorWeight, forwards])

  return useMemo(() => ({ loading, price, pricePerUnit }), [loading, price, pricePerUnit])
}
...
export function useMakeOrder() {
  ...
  return useCallback(
    async (amount, toBonded = true, minReturn = 0) => {
      try {
        ...
        return await (toBonded
          ? marketMakerContract.makeBuyOrder(collateralAddress, amount, 0, {
              gasLimit: 650000,
              value: 0,
            })
          : marketMakerContract.makeSellOrder(collateralAddress, amount, 0, {
              gasLimit: 850000,
            }))
      } catch (err) {
        throw new Error(err.message)
    ...
    [collateralAddress, marketMakerContract]
  )
}
