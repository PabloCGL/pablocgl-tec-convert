...
export function getTributeValues(){
  const marketMakerContract = useKnownContract('MARKET_MAKER')
  ...
  const getTribute = useCallback(async () => {
    try {
      ...
      setEntryTribute((buyFeePct/PCT_BASE))
      setExitTribute((sellFeePct/PCT_BASE))
    } catch (err) {
      console.log(err)
      throw new Error(err.message)
    }
...
export function getAmountReceived(inputValueRecipient, tributePct){
  ...
  const getAmount = useCallback(async () => {
    try {
      ...
      //Remove commas before parsing in case the number is over 1,000.00
      let inputNum = Number(inputValueRecipient.replaceAll(",", ""))

      let withdrawableAmount = inputNum * (1- tributePct)

      setEstAmount(withdrawableAmount)

    } catch (err) {
      console.log(err)
      throw new Error(err.message)
    }
  }, [inputValueRecipient, tributePct])
...
export function getMinimumWithSlippage(estimatedAmount, slippage){
  ...
  const getAmount = useCallback(async () => {
    try {
      ...
      setSlipAmount(Number(estimatedAmount* (1-slippage)))
